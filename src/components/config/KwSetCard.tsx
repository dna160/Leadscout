...
import type { KeywordSet } from "@/domain/run"
...
export function KwSetCard({ kwSet, onUpdate, onDelete }: KwSetCardProps) {
  ...
  async function patchSet(data: Partial<KeywordSet>) {
    setSaving(true)
    try {
      const res = await fetch(`/api/keyword-sets/${kwSet.id}`, {
        method: "PATCH",
    ...
    <div className="bg-white shadow-sm rounded-lg p-4 flex flex-col gap-3">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <Badge className={SEGMENT_COLOURS[kwSet.segment]}>
            {SEGMENT_LABELS[kwSet.segment]}
          </Badge>
