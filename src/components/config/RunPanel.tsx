...
import type { ScrapeRun } from "@/domain/run"
  ...
  async function pollRuns(runId: string): Promise<void> {
    ...
    for (let i = 0; i < MAX_POLLS; i++) {
      ...
      try {
        ...
        if (!res.ok) continue
        const updated = (await res.json()) as ScrapeRun[]
        setRuns(updated)
  ...
  async function handleRun() {
    ...
    try {
      const res = await fetch("/api/scrape", {
        ...
        body: JSON.stringify({ maxPlacesPerSearch: maxPlaces }),
      })
      ...
      if (runsRes.ok) {
        const refreshed = (await runsRes.json()) as ScrapeRun[]
        setRuns(refreshed)
      }
