...
import type { KeywordSet, ScrapeRun, City } from "@/domain/run"
...
export default function ConfigPage() {
  ...
  useEffect(() => {
    async function fetchAll() {
      setLoading(true)
      try {
        const [kwRes, cityRes, runRes] = await Promise.all([
          fetch("/api/keyword-sets"),
          fetch("/api/cities"),
          fetch("/api/runs"),
        ])
        if (kwRes.ok) setKwSets((await kwRes.json()) as KeywordSet[])
        if (cityRes.ok) setCities((await cityRes.json()) as City[])
        if (runRes.ok) setRuns((await runRes.json()) as ScrapeRun[])
      } catch (err) {
        console.error(err)
      } finally {
        setLoading(false)
  ...
  async function handleAddSet(e: React.FormEvent) {
    ...
    setAddSaving(true)
    try {
      const res = await fetch("/api/keyword-sets", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ segment: newSegment, label: trimmedLabel, keywords }),
      })
      if (!res.ok) throw new Error(`POST failed: ${res.status}`)
      const created = (await res.json()) as KeywordSet
